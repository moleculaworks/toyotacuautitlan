...
- [ ] Cargar el resto de los 20 modelos restantes (datos desde fichas técnicas + toyota.mx, precio siempre desde toyota.mx; imágenes las trabaja Raúl en Photoshop) — proceso ya probado con Corolla HEV, Camry HEV, Yaris Sedán y Yaris Sedán HEV: borrador en JSON (`JSON Modelos/`) → revisión visual → aprobación → Sanity → publicar
  ...
  - [x] **Yaris Sedán HEV publicado en Sanity** (16 de septiembre de 2026) — cuarto modelo del catálogo (slug `yaris-sedan-hev`, documento `ca984df4-2f37-46eb-8784-cd83b8fc7621`), par HEV del Yaris Sedán con el carrusel de `versiones` duplicado (mismas 6 tarjetas e imágenes reutilizadas por referencia del documento `yaris-sedan`, sin volver a subirlas). Colores del visor 360° leídos de los atributos `data-color1` del DOM (6: Blanco, Rojo, Plata, Escarlata, Gris, Negro — sin Blanco Perlado, a diferencia del Yaris Sedán de gasolina). Primer modelo del catálogo con **36 ángulos por color** en vez de 16 (216 imágenes de 360°, detectado en vivo contra toyota.mx, no asumido) y con imágenes fuente en PNG con transparencia en vez de JPG opaco (compuestas sobre fondo blanco y ajustadas a 1200×800 sin recortar, ya que su proporción real no es 3:2 exacta como en los modelos anteriores). **Sin galería de interior** (decisión del proyecto, no imágenes faltantes) — se ajustó `components/modelo/Galeria.tsx` para ocultar el botón "Interior" automáticamente cuando ese arreglo viene vacío en vez de mostrar un grid en blanco (cambio de código pendiente de commit/push, ver nota abajo). 226 imágenes totales subidas vía `sanity exec` + `client.assets.upload()` (mismo mecanismo que los 3 modelos anteriores). Pendiente: commitear/subir el fix de `Galeria.tsx` y hacer deploy en Vercel para que la página exista en el sitio (ver `AGENTS.md`).
