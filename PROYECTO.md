...
- [ ] **Corolla HEV — pendiente inmediato:** borrador de datos y textos ya completo y aprobado en `JSON Modelos/corolla-hev.json`. Falta: (1) que Raúl produzca las fotos propias (hero, tarjeta, destacado, rendimiento, colores/360°, galería — carpeta `Imagenes por Modelo/corolla-hev/` ya creada, las 6 fotos de versiones ya están listas); (2) cargarlo a Sanity como documento nuevo una vez haya imágenes. Ver Obsidian `catalogo-de-modelos.md`.
- [ ] Cargar el resto de los modelos (datos desde fichas técnicas + toyota.mx, precio siempre desde toyota.mx; imágenes las trabaja Raúl en Photoshop) — proceso ya probado con Corolla HEV: borrador en JSON (`JSON Modelos/`) → revisión visual en `/preview-borrador` → aprobación → Sanity
