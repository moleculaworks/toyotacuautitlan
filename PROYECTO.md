...
- [ ] Cargar el resto de los 22 modelos (datos desde fichas técnicas + toyota.mx, precio siempre desde toyota.mx; imágenes las trabaja Raúl en Photoshop) — proceso ya probado con Corolla HEV: borrador en JSON (`JSON Modelos/`) → revisión visual → aprobación → Sanity → publicar
  - [ ] **Camry HEV — en progreso (16 de septiembre de 2026):** borrador de datos/textos completo y aprobado (`JSON Modelos/camry-hev.json`), revisado en `/preview-borrador?json=camry-hev`. Modelo único, sin par CVT/gasolina (en México el Camry solo existe como HEV). Colores confirmados contra toyota.mx en vivo (5: Blanco, Blanco Perlado, Gris Metálico, Grafito, Rojo — la ficha técnica lista más pero no están fotografiados en el configurador real). Raúl está produciendo las imágenes propias ahora; carpeta `Imagenes por Modelo/camry-hev/` ya creada. Pendiente: imágenes → cargar a Sanity → publicar.
